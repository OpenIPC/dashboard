...
export const AppStateProvider: React.FC<AppStateProviderProps> = ({ children }) => {
  ...
  const mediamtxStartedRef = useRef(false);
  const mediamtxStartPromiseRef = useRef<Promise<void> | null>(null);
  const prewarmedCamerasRef = useRef<Set<number>>(new Set());

  const ensureMediaMtxStarted = useCallback(async () => {
    if (!isTauriAvailable()) {
      return;
    }

    if (mediamtxStartedRef.current) {
      return;
    }

    if (mediamtxStartPromiseRef.current) {
      await mediamtxStartPromiseRef.current;
      return;
    }

    const startPromise = (async () => {
      try {
        const result = await invoke<string>('mediamtx_start');
        mediamtxStartedRef.current = true;
        console.log('AppStateContext: MediaMTX start result:', result);
      } catch (error) {
        console.warn('AppStateContext: Failed to start MediaMTX:', error);
      } finally {
        mediamtxStartPromiseRef.current = null;
      }
    })();

    mediamtxStartPromiseRef.current = startPromise;
    await startPromise;
  }, []);
  ...
  const prewarmCameraStreams = useCallback(async (camera: Camera) => {
    ...
    try {
      ...
      const verifyPathReady = async (pathName: string) => {
        try {
          await new Promise(resolve => setTimeout(resolve, 500));
          const ready = await invoke<boolean>('check_mediamtx_path_ready', { pathName });
          if (!ready) {
            console.warn(`AppStateContext: MediaMTX path ${pathName} not ready after warmup check`);
          } else {
            console.log(`AppStateContext: MediaMTX path ${pathName} confirmed ready`);
          }
        } catch (error) {
          console.warn(`AppStateContext: Failed to verify MediaMTX path ${pathName}:`, error);
        }
      };

      const baseName = `cam${camera.id}`;
      await verifyPathReady(`${baseName}_1`);
      await verifyPathReady(`${baseName}_0`);

      console.log('AppStateContext: Prepared MediaMTX streams for camera', camera.id);
    } catch (error) {
      prewarmedCamerasRef.current.delete(camera.id);
      console.warn('AppStateContext: Failed to prepare MediaMTX for camera', camera.id, error);
    }
  }, [ensureMediaMtxStarted]);
