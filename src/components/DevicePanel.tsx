...
const DevicePanel: React.FC = () => {
  ...
  const handleMoveCameraToGroup = async (camera: Camera, groupId: number | null) => {
  ...
  useEffect(() => {
    registerDefaultCameraContextMenuHandlers({
      ...
      onFileManager: (camera: Camera) => {
        console.warn('File manager action is not implemented yet for camera', camera.id);
      },
      onSSH: (camera: Camera) => {
        console.warn('SSH action is not implemented yet for camera', camera.id);
      },
