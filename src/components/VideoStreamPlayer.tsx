...
import type { RtspHandshakeResponse } from '../services/rtsp';
...
const VideoStreamPlayer: React.FC<VideoStreamPlayerProps> = ({
  ...
  const lastHandshakeRef = useRef<RtspHandshakeResponse | null>(null);
  ...
  overlayTimerRef.current = setTimeout(() => {
    ...
    const startWebRTC = async () => {
      ...
      try {
        ...
        type HandshakeMeta = { result: RtspHandshakeResponse; url: string };

        let handshakeTransportHint: string | undefined;
        let handshakeSessionHint: string | undefined;
        let handshakeHintsApplied = false;

        const applyHandshakeMeta = (meta: HandshakeMeta | null, source: 'cached' | 'fresh'): void => {
          if (!meta) {
            return;
          }

          lastHandshakeRef.current = meta.result;
          handshakeSessionHint =
            meta.result.session ?? meta.result.video.responseHeaders['session'];
          handshakeTransportHint = meta.result.video.responseHeaders['transport'];

          if (!handshakeHintsApplied && debug) {
            if (source === 'fresh') {
              meta.result.log.forEach(entry => logDebug(`[RTSP] ${entry}`));
            }
            logDebug(
              `[RTSP] Handshake (${source}) session=${handshakeSessionHint ?? 'n/a'}, transport=${handshakeTransportHint ?? 'n/a'}, url=${meta.url}`,
            );
          }

          handshakeHintsApplied = true;
        };

        if (lastHandshakeRef.current) {
          applyHandshakeMeta({
            result: lastHandshakeRef.current,
            url: lastHandshakeRef.current.baseUri ?? 'cached',
          }, 'cached');
        }

        if (isTauriAvailable()) {
          const transportPreference = useHdQuality ? 'tcp' : 'udp';
          const handshakeDeadlineMs = 700;
          const shouldAwaitQuickHints = !handshakeHintsApplied;

          const handshakePromiseLocal = (async (): Promise<HandshakeMeta | null> => {
            try {
              const localHandshakeUrl = `rtsp://127.0.0.1:8554/${name}`;
              let handshakeUrl = localHandshakeUrl;

              try {
                const resolvedSource = await resolveStreamSource(name);
                if (resolvedSource) {
                  handshakeUrl = resolvedSource;
                  if (debug) {
                    logDebug(`[RTSP] Resolved source for ${name}: ${resolvedSource}`);
                  }
                } else if (debug) {
                  logDebug(`[RTSP] Using default local MediaMTX source for ${name}`);
                }
              } catch (resolveError) {
                if (debug) {
                  logDebug(
                    `[RTSP] Failed to resolve source for ${name}: ${resolveError instanceof Error ? resolveError.message : String(resolveError)}`,
                  );
                }
              }

              const attemptHandshake = async (url: string): Promise<RtspHandshakeResponse | null> => {
                if (debug) {
                  logDebug(`[RTSP] Handshake attempt via ${url}`);
                }
                return performRtspHandshake({
                  url,
              ...
              let handshakeResult = await attemptHandshake(handshakeUrl);

              if (!handshakeResult && handshakeUrl !== localHandshakeUrl) {
                if (debug) {
                  logDebug(`[RTSP] Handshake via ${handshakeUrl} failed, retrying local proxy`);
                }
                handshakeResult = await attemptHandshake(localHandshakeUrl);
                handshakeUrl = localHandshakeUrl;
              }
              ...
              return { result: handshakeResult, url: handshakeUrl };
            } catch (handshakeError) {
              if (debug) {
                logDebug(
                  `[RTSP] Handshake flow failed: ${handshakeError instanceof Error ? handshakeError.message : String(handshakeError)}`,
                );
              }
              return null;
            }
          ...
          handshakePromiseLocal
            .then(meta => {
              applyHandshakeMeta(meta, 'fresh');
            })
            .catch(() => {
              /* already logged */
            });

          if (shouldAwaitQuickHints) {
            const quickMeta = await Promise.race<HandshakeMeta | null>([
              handshakePromiseLocal,
              new Promise<null>(resolve => {
                setTimeout(() => resolve(null), handshakeDeadlineMs);
              }),
            ]);

            applyHandshakeMeta(quickMeta, 'fresh');
          }
        }
        ...
        pc.oniceconnectionstatechange = () => {
          ...
          if (isActive && (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed')) {
        ...
        pc.ontrack = (ev) => {
          ...
          if (stream) {
            ...
            if (ev.track.kind === 'video' && isActive) {
              setConnectionMethod('WebRTC');
              hideConnectingOverlay();
              setIsConnecting(false);
              setErrorMessage(null);
        ...
        if (debug) {
          logDebug(`Offer SDP preview includes HEVC: ${/H265|HEVC/i.test(offer.sdp)}`);
        }
        ...
        try {
          ...
          if (handshakeSessionHint) {
            whepPayload.rtspSession = handshakeSessionHint;
          }
          if (handshakeTransportHint) {
            whepPayload.rtspTransport = handshakeTransportHint;
          }
    ...
    async function startStream(): Promise<void> {
      ...
      try {
        ...
        logDebug(`WebRTC start failed (attempt ${attemptNumber}), retrying in ${retryDelay}ms`);
