...
const Cameras: React.FC = () => {
  ...
  const handleFileManager = (camera: Camera) => {
    ...
    console.log('File manager for camera:', camera.name);
    // Можно открыть модальное окно с файловым менеджером
  };

  const handleSSH = (camera: Camera) => {
    // TODO: Реализовать SSH терминал
    console.log('SSH terminal for camera:', camera.name);
    // Можно открыть модальное окно с SSH терминалом
  };
