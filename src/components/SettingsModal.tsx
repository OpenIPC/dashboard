...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        const loadedSettings = { ...settings, ...(savedSettings as any) };
        // Ensure current language is reflected in settings
        loadedSettings.language = currentLanguage;
        ...
        if (localSettings) {
          const loadedSettings = { ...settings, ...JSON.parse(localSettings) };
          // Ensure current language is reflected in settings
          loadedSettings.language = currentLanguage;
          setSettings(loadedSettings);
        } else {
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      const successMessage = settings.language !== currentLanguage 
        ? (settings.language === 'en' ? 'Settings saved successfully!' : 'Настройки успешно сохранены!')
