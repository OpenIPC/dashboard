...
const Dashboard: React.FC = () => {
  ...
  const resolveStreamPaths = (camera: Camera, hdFallback?: string) => {
  ...
  const resolveCameraRtspUrls = useCallback(async (camera: Camera): Promise<{ hdUrl: string; sdUrl: string }> => {
    if (camera.streamUrl && camera.streamUrl.startsWith('rtsp://')) {
      try {
        const baseUrl = new URL(camera.streamUrl);
        const { hdPath, sdPath } = resolveStreamPaths(camera, baseUrl.pathname);
        ...
        console.warn('Dashboard: Failed to parse streamUrl, falling back to manual construction', error);
      }
    }
    ...
    const { hdPath, sdPath } = resolveStreamPaths(camera);
    return {
      hdUrl: `${base}${hdPath}`,
      sdUrl: `${base}${sdPath}`,
    };
  ...
  const assignCameraToCell = useCallback(async (camera: Camera, cellIndex: number) => {
    ...
    try {
      const { hdUrl, sdUrl } = await resolveCameraRtspUrls(camera);
  ...
  }, [resolveCameraRtspUrls, fullscreenCell]);
