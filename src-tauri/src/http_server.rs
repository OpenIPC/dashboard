...
pub async fn start_http_server() -> Result<bool, String> {
    ...
    let recordings_path = recordings_dir.to_string_lossy().to_string();
        ...
        if parts.len() >= 2 {
            ...
            if method == "GET" || method == "HEAD" {
                if path == "/" {
                    ...
                    let decoded_file_path = match decode(file_path) {
                        Ok(decoded) => decoded.into_owned(),
                        Err(err) => {
                            eprintln!(
                                "Failed to decode URL path '{}': {} – using raw fallback",
                                file_path, err
                            );
                            file_path.replace('+', " ")
                        }
                    ...
                    let sanitized_path = decoded_file_path.trim_start_matches('/');

                    if Path::new(sanitized_path).is_absolute() || sanitized_path.contains("..") {
                        eprintln!(
                            "Rejected suspicious path '{}' after decoding '{}'",
                    ...
                    let base_path = Path::new(recordings_path);
