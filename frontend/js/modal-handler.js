...
(function(window) {
    ...
    window.AppModules.createModalHandler = function(App) {
            ...
            if (addScreenshotBtn) {
                addScreenshotBtn.addEventListener('click', async () => {
                    try {
                        ...
                        const dataUrls = await window.api.openAndReadImageFiles();
        ...
        return { 
            init,
            // Forward optional camera object so the camera modal can be opened in edit mode
            openAddModal: (camera) => cameraHandler.openAddModal(camera),
