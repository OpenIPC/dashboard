...
Статус: 🟡 первый UI-пакет в работе.
...
- `ArchiveView.qml` больше не содержит встроенный список результатов и корневой календарь;
...
- вынести player controls в отдельный компонент;
- привести export controls к той же компонентной структуре;
- добавить сортировку/фильтр по source, если реальные сценарии покажут, что ручные и event-записи нужно разделять в UI.
...
- предупреждения, если запись идет в недоступный или переполненный каталог.
...
- связать будущие analytics-triggered clips с тем же каталогом и UI-архивом.
